...
const SectionCard = ({ icon: Icon, title, description, children, onSave, saving }: { icon: any; title: string; description: string; children: React.ReactNode; onSave?: () => void; saving?: boolean }) => (
  <div className="rounded-2xl p-6" style={{ background: "hsl(0 0% 100% / 0.03)", border: "1px solid hsl(0 0% 100% / 0.06)" }}>
    <div className="flex items-center gap-3 mb-1">
      <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "hsl(270 70% 55% / 0.15)" }}>
    ...
    {onSave && (
      <div className="mt-5 flex justify-end">
        <button onClick={onSave} disabled={saving} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all hover:opacity-90 disabled:opacity-50" style={{ background: "hsl(270 70% 55%)", color: "hsl(0 0% 100%)" }}>
    ...
    <div className="max-w-4xl space-y-6">
      ...
      {activeTab === "general" && (
        <>
          ...
          <div className="p-4 rounded-xl text-sm flex items-center justify-between gap-4"
               style={{ background: "hsl(270 70% 55% / 0.1)", border: "1px solid hsl(270 70% 55% / 0.3)", color: "hsl(0 0% 100% / 0.8)" }}>
            <div>
              <div className="font-bold" style={{ color: "hsl(0 0% 100%)" }}>Wrapped Designer</div>
              <div className="text-xs mt-0.5" style={{ color: "hsl(0 0% 100% / 0.6)" }}>
                Slide-Toggles, Hintergründe, Bilder & Fallback-Song pro Jahr findest du jetzt im eigenen Menüpunkt „Year-in-Review".
              </div>
            </div>
            <a href="/admin/wrapped" className="text-xs font-bold px-3 py-2 rounded-md whitespace-nowrap"
