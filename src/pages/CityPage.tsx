...
const MediaWidget = ({ eventId, title, showTitle, type, externalUrls = [], galleryConfig }: { eventId: string; title: string; showTitle: boolean; type: "photos" | "videos"; externalUrls?: string[]; galleryConfig?: GalleryConfig }) => {
  ...
  return (
    <div className="pt-6">
      ...
      {/* Video Slideshow – multi-tile grid, fades to next when current ends */}
      {viewMode === "slideshow" && isVideo ? (
        <VideoSlideshow files={allFiles} cols={cols} aspectClass={aspectClass} gridColsClass={gridColsClass} />
      ) : viewMode === "slideshow" && !isVideo ? (
        <div>
    ...
    {headerImage && (
      <motion.img
        ...
        style={{
          ...
          ...(event.soldOut ? { filter: "grayscale(100%)", opacity: 0.7 } : {}),
        }}
...
const CityPage = () => {
  ...
  // Apply header image as page background
  useEffect(() => {
    const root = document.querySelector('.pp-bg') as HTMLElement | null;
    if (!root) return;
    if (headerImageUrl) {
      root.style.setProperty('--pp-bg-image', `url(${headerImageUrl})`);
      root.classList.remove('pp-bg--no-image');
    } else {
      root.style.removeProperty('--pp-bg-image');
      root.classList.add('pp-bg--no-image');
    }
  }, [headerImageUrl]);
